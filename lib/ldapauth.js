...
function LdapAuth(opts) {
  ...
  if (opts.groupSearchBase && opts.groupSearchFilter) {
    ...
    this._getGroups = this._findAllGroups;
    this._bypassedGroupsByUser = [];
  } else {
...
LdapAuth.prototype._findGroupsRecursively = function(member, callback, user) {
  var self = this;
  if (!member) {
    return callback(new Error('no member'));
  }

  if (user) {
    self._bypassedGroupsByUser = self._bypassedGroupsByUser || [];
    self._bypassedGroupsByUser[user] = self._bypassedGroupsByUser[user] || [];
  }

  self._findGroups(member, function(err, groups) {
    if (err) {
      return callback(err);
    }

    var groupsToCheck = user
      ? groups.filter(g => !self._bypassedGroupsByUser[user].includes(g.cn))
      : groups;

    async.map(groupsToCheck, (group, cb) => self._findGroupsRecursively(group, cb, user), function(error, res) {
      return callback(error, groups.concat.apply(groupsToCheck, res));
    });

    if (user) {
      self._bypassedGroupsByUser[user] = self._bypassedGroupsByUser[user].concat(groupsToCheck.map(g => g.cn));
    }
  });
};

/**
 * Find ALL groups for given user
 *
 * @private
 * @param {Object} user - The LDAP user object
 * @param {resultCallback} callback - Result handling callback
 * @returns {undefined}
 */
LdapAuth.prototype._findAllGroups = function(user, callback) {
  var self = this;
  if (!user) {
    return callback(new Error('no user'));
  }
  ...
  self._bypassedGroupsByUser = self._bypassedGroupsByUser || [];
  self._bypassedGroupsByUser[user.cn] = [];

  self._findGroupsRecursively(user, function(err, groups) {
    user._groups = groups;
