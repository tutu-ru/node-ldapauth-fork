...
LdapAuth.prototype._findGroupsRecursively = function(member, callback) {
  var self = this;
  if (!member) {
  ...
  self._findGroups(member, function(err, groups) {
    ...
    async.map(groups, self._findGroupsRecursively.bind(self), function(error, res) {
      return callback(error, groups.concat.apply(groups, res));
    });
  });
};
